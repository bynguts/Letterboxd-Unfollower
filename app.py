...
st.title("🎬 Letterboxd Unfollower Dashboard")
st.write("Visualize your followers & unfollowers, and search/filter your connections.")
...
if check_button and username:
    ...
    # --------------------------
    # Tabs for lists with search/filter
    # --------------------------
    tabs = st.tabs(["Doesn't Follow You Back", "You Don't Follow Back"])
    
    def display_user_list(user_list, highlight_color):
        search_term = st.text_input("Search username:", key=highlight_color)
        filtered = [u for u in user_list if search_term.lower() in u["username"].lower()] if search_term else user_list
        for u in filtered:
            cols = st.columns([1, 4])
            with cols[0]:
                if u.get("avatar"):
    ...
    with tabs[0]:
        if unfollowers:
            ...
            filtered_unfollowers = display_user_list(unfollowers, "#FF4C4C")
            df_unfollowers = pd.DataFrame([u["username"] for u in filtered_unfollowers], columns=["username"])
    ...
    with tabs[1]:
        if unfollowing:
            ...
            filtered_unfollowing = display_user_list(unfollowing, "#4C9EFF")
            df_unfollowing = pd.DataFrame([u["username"] for u in filtered_unfollowing], columns=["username"])
    ...
    df_chart = pd.DataFrame({
        "Metric": ["Followers", "Following", "Doesn't Follow Back", "You Don't Follow Back"],
        "Count": [len(followers), len(following), len(unfollowers), len(unfollowing)]
    })
    ...
    chart = alt.Chart(df_chart).mark_bar().encode(
        ...
        color=alt.Color('Metric', 
                        scale=alt.Scale(domain=["Followers","Following","Doesn't Follow Back","You Don't Follow Back"],
                                        range=["#4C9EFF", "#4CAF50", "#FF4C4C", "#2196F3"]))
    ).properties(width=700, height=400, title="Followers & Unfollowers Summary")
